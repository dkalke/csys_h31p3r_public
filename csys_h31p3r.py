...
class MainGui(wx.Frame):
    ...
    def __init__(self, parent):
        wx.Frame.__init__(self, parent, id=wx.ID_ANY, title=u"CsYs_h31P3R v0.1.7 Build 180911       By P1us. N ",
        ...
        ################################################################################################################
        self.app_config = config.init_config()
        self.console_login = Login(self.app_config)
        self.student_login = Login(self.app_config)
        ...
        try:
            with open('user.json', 'r+') as f:
                try:
                    ...
                    try:
                        self.student_id = self.user_config['id']
                        if self.user_config['remember_all']:
                            new_id = self.student_id
                            if len(new_id) < 8:
                                new_id = (new_id + '88888888')[0:8]
                            elif len(new_id) > 8:
                                new_id = new_id[0:8]
                            try:
                                f = Fernet(str.encode(new_id + '888888888888888888888888888' + new_id + "="))
                                self.student_pw = bytes.decode(f.decrypt(str.encode(self.user_config['pw'])))
                            except InvalidToken:
                                pass
                        else:
                            self.student_pw = ""
        ...
        self.SetForegroundColour(wx.Systemconfigs.GetColour(wx.SYS_COLOUR_INACTIVECAPTIONTEXT))
        self.SetBackgroundColour(wx.Systemconfigs.GetColour(wx.SYS_COLOUR_INACTIVEBORDER))
        ...
        self.m_system_status_text.SetBackgroundColour(wx.Systemconfigs.GetColour(wx.SYS_COLOUR_BTNTEXT))
        ...
        self.m_save_help_course_config = wx.MenuItem(self.m_menu3, wx.ID_ANY, u"保存清單", wx.EmptyString, wx.ITEM_NORMAL)
        self.m_menu3.Append(self.m_save_help_course_config)

        self.m_load_help_course_config = wx.MenuItem(self.m_menu3, wx.ID_ANY, u"讀取清單", wx.EmptyString, wx.ITEM_NORMAL)
        self.m_menu3.Append(self.m_load_help_course_config)
        ...
        self.Bind(wx.EVT_MENU, self.on_save_help_course_config, id=self.m_save_help_course_config.GetId())
        self.Bind(wx.EVT_MENU, self.on_load_help_course_config, id=self.m_load_help_course_config.GetId())
    ...
    def app_version_check(self):
        ...
        online_version_list = requests.get('https://docs.google.com/spreadsheets/d/'+sheet_id+
    ...
    def verify_course_list_old(self, event):
        if self.student_login.login_status:
            ...
            self.on_save_help_course_config(event)
        else:
    ...
    def do_get_course_list_from_course_query_system(self):
        ...
        for i in range(1, len(course_list_raw), 1):
            self.online_course_data[course_list_raw[i].split('|')[6]] = course_list_raw[i].split('|')[10]
    ...
    def do_login(self, event):
        ...
        if login_init_package['result']:
            ...
            if self.student_id and self.student_pw:
                ...
                if self.student_login.login_status:
                    self.course_request = Course(self.student_login.login_config, self.student_login.get_session(),
                                                 self.student_id)
    ...
    def on_save_help_course_config(self, event):
    ...
    def on_load_help_course_config(self, event):
        self.do_load_help_course_list()
        event.Skip()
